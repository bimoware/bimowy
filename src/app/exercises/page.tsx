...
function Card({ exercise }: { exercise: ExerciseGenerator }) {
  ...
  return (
    <Link href={`/exercises/${exercise.id}`} className={`w-fit h-fit bg-neutral-700/20 rounded-xl
    p-4 px-8
    ...
    ${randomRotationClass}`}>
      {exercise.recent
        && <span className='absolute px-3 py-1 -m-8 group-hover:-rotate-5 group-hover:-translate-1
        transition-transform
        bg-indigo-800/70 rounded-full -rotate-2 font-bold
        '>NEW</span>
      }
      <h4 className="self-center text-2xl font-bold">{exercise.name}</h4>
      <span className="self-center text-basic text-[0em] group-hover:text-sm transition-all text-neutral-50/50">{exercise.desc}</span>
      {/* Tags */}
