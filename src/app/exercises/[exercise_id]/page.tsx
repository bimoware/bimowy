...
export default function ExercisePage() {
  ...
  function collectInputs(
    ...
    const traverse = (
      nodes: GeneratedExercise['context']
    ) => {
      nodes.forEach((node) => {
        if (node.type === 'input') {
  ...
  function focusFirstIncorrectInput() {
    ...
    const firstIncorrectIndex = corrections.findIndex(
      (c) => !c || !c.correct
    )
  ...
  function handleCorrection(res: ExerciseCorrection[]) {
    setCorrections((prev) =>
      res.map((answer, i) => ({
        ...
        correctOnFirstTry:
          prev[i]?.correctOnFirstTry ?? answer.is_correct
      }))
    )
  ...
  function getIsInputDisabled(index: number) {
    ...
    return (
      correction.correctOnFirstTry || correction.correct
    )
  }
    ...
    switch (node.type) {
      case 'p':
        return (
          <p
            key={key}
            className='inline-block space-x-2 items-center'
          >
        ...
        return (
          <input
            ...
            className={`text-center rounded-md px-2 py-1 bg-neutral-800 w-fit max-w-24
							${
                !correction
                  ? 'focus:outline-2 outline-1 outline-white'
                  : correction.correct
