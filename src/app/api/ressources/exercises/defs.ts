import globalTags from "@api/ressources/tags"
import { Language, LocaleString, LocaleStringRecord } from "@api/main"
// Contexts
...
type ExerciseData = {
	...
	tags: typeof globalTags[number]["id"][]
}
...
> {
	...
	setTags(tags: typeof globalTags[number]["id"][]) {
	...
	serialize(lang: Language) {
		...
		const tags = this.rawData.tags.map(t => globalTags.find(globalT => globalT.id == t)!.names[lang])
