...
export async function POST(
	...
	const exercise = await resourcesManager.fetch(exerciseId)! as AnyExerciseBuilder
	...
	// Main
	const nQuestions = options[DEFAULT_N_QUESTIONS_ID]
	const exercises = Array.from({
		length: typeof nQuestions === "number" ? nQuestions : DEFAULT_N_QUESTIONS_OPTION.config.defaultValue
	}).map(() => exercise.generate(options, lang))

	return NextSuccess(exercises)
}

export type ExercisesGenerateRouteResult = ReturnType<AnyExerciseBuilder["generate"]>[]
