...
import { ExercisesGenerateRouteResult } from "@api/ressources/exercises/generate/route";
import { ExercisesValidateRouteResult } from "@api/ressources/exercises/validate/route";
...
import { ContextElement, ContextSection, ExerciseBuilder } from "@api/ressources/exercises/defs";
...
export default function ExerciseClientPage({ exerciseData, exercise_id, locale }: {
	...
	}), {} as { [A in keyof typeof exerciseData["options"]]: typeof exerciseData["options"][A]["defaultValue"] })
	const [userOptionValues, setUserOptionValues] = useState(defaultOptions)
	...
	const actions = {
		...
		fetchExercises: async function () {
			const url = new URL('/api/ressources/exercises/generate', window.location.origin)
		...
		correctExercise: async function () {
			...
			const url = new URL('/api/ressources/exercises/validate', window.location.origin)
			url.searchParams.append('id', exercise_id)
